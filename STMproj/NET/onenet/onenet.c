...
#define PROID		"cy_smarthome"

#define AUTH_INFO	"cy_smarthome_info"

#define DEVID		"cy_smarthome"
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				else
				{
					...
					//json_value->string是键, valuestring是值
					UsartPrintf(USART_DEBUG,"json_value: [%s]\n",json_value->string);
					UsartPrintf(USART_DEBUG,"json_value: [%s]\n",json_value->valuestring);
					//如果target字段为"LED",则进入LED控制判断
					if(strstr(json_value->valuestring,"LED") != NULL)
					{
						//取value字段的值
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)LED0 = 0;//如果value字段的值为非0, 则开灯
						else LED0 = 1;//如果value字段的值为0, 则关灯
					}
					//如果target字段为其他任何字段,则进入警报器控制判断
					else
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)alarmFlag = 1;//如果value字段的值为非0, 则打开报警器
						else alarmFlag = 0;//如果value字段的值为0, 则关闭报警器
...
//					}
				...
				cJSON_Delete(json);//删除json包,节约内存
			}
		break;
